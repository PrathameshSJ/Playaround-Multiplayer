...
@onready var cam_pitch = $CamRoot/CamPitch
...
func _physics_process(delta: float) -> void:
	...
	if is_multiplayer_authority() or NetworkManager.isSinglePlayer:
		...
		NetworkManager.broadcast_transform(self)
	else:
	...
	# --- Movement ---
	var forward = -cam_root.transform.basis.z
	var right = cam_root.transform.basis.x
...
func rotate_to_direction(target_pos: Vector3) -> void:
	...
	if dir.length() > 0.01:
		...
		#cam_root.rotation.y = target_rot
